 ...
 */
@Slf4j
public class OrderPermit1Handler implements Handler {
    ...
    public void handle(TransactionContext context, StateMachine stateMachine) {
        ...
        orderBO.setOrderProcessState(OrderPermitState.DELIVERED_PERMIT.getCode());

        log.info("[{}],permit=[{}]", this.getClass().getSimpleName(), orderBO);

        String leaderSuggestion = (String) context.getData(OrderContextConstants.ORDER_PERMIT_1_SUGGESTION);
